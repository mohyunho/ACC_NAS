...
def main():
    ...
    mutate_filename = os.path.join(log_dir_path, 'mute_log_%s_%s_%s_%s.csv' % (pop_size, n_generations, obj, trial))
    ...
    for idx, row in last_gen_df.iterrows():
        ...
        if idx == 0 or ([int(row['params_1'] ),int(row['params_2'] ),int(row['params_3'] ),int(row['params_4'] )] not in params_hist_lst):
            print ("path1", idx)
            ...
            params_hist_lst.append([int(row['params_1'] ),int(row['params_2'] ),int(row['params_3'] ),int(row['params_4'] )])

        else:
            print ("path2", idx)
            l_index = params_hist_lst.index([int(row['params_1'] ),int(row['params_2'] ),int(row['params_3'] ),int(row['params_4'] )])

            rms = last_gen_rmse_lst[l_index]
            score = last_gen_score_lst[l_index]
            num_tran_params = last_gen_numparams_lst[l_index]
            training_time = last_gen_traintime_lst[l_index]
            inference_time = last_gen_infertime_lst[l_index]
    ...
    last_gen_df['train_time'] = last_gen_traintime_lst
    last_gen_df['infer_time'] = last_gen_infertime_lst
    last_gen_df['num_params'] = last_gen_numparams_lst
    last_gen_df['RMSE'] = last_gen_rmse_lst
    last_gen_df['score'] = last_gen_score_lst
    ...
    output_filepath = os.path.join(log_dir_path, 'inference_last_gen_%s_%s_%s_%s.csv' % (pop_size, n_generations, obj, trial))
    last_gen_df.to_csv(output_filepath, index=False)
