...
from utils.accel_cnn_task_combined import Task
from utils.accel_cnn_network_initial_corr import corr_coeff
...
def eaSimple(population, toolbox, cxpb, mutpb, ngen, sel_op, batch_size, train_sample_array, stats=None,
    ...
    # compute architecture score for invalid_ind
    # curve fitting to analyze correlation between validation RMSE and architecture score
    coeff_array = corr_coeff(invalid_ind, fit_lst, batch_size, train_sample_array)
   
    # save coefficient of the curve to file
    with open('coeff.npy', 'wb') as f:  # Python 3: open(..., 'wb')
        np.save(f, coeff_array)
    ...
    if sel_op == "nsga2":
        ...
        for gen in range(1, ngen + 1):
            ...
            hv = hypervolume(paretofront_hv,[30.0, 200000.0])
            print ("hv",hv)
...
class GeneticAlgorithm:
    def __init__(self, batch_size, train_sample_array, task: Task, population_size: int, n_generations: int, cx_probability: float,
    ...
    def run(self):
        ...
        pop, log = eaSimple(
            ...
            train_sample_array = self.train_sample_array,
            stats=stats,
            halloffame=hof,
            paretofront=prtf,
            verbose=True,
